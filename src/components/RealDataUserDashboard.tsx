...
export function RealDataUserDashboard({ session }: UserDashboardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { 
            icon: DollarSign, 
            label: 'Total Income', 
            value: `$${(dashboardStats?.totalIncome || 0) / 1000000}M`, 
            color: 'text-amber-500',
            trend: '+3.5%'
          },
        ].map((item, i) => (
          <motion.div key={item.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
            <GlassCard className="p-4 hover:bg-card/75 transition-all">
      ...
      {/* My Appointments - Real data from backend */}
      {userAppointments.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <GlassCard className="p-6">
            <h3 className="text-base font-semibold text-foreground mb-4">My Appointments</h3>
            <div className="space-y-3">
              {userAppointments.slice(0, 5).map((appointment: any, i: number) => (
                <div key={i} className="p-4 bg-card/40 backdrop-blur-sm rounded-xl border border-border space-y-3">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{appointment.doctor || 'Dr. TBD'}</p>
                      <p className="text-xs text-muted-foreground">
                        {appointment.department || 'General'} · {new Date(appointment.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })} at {appointment.time}
                      </p>
                    </div>
                  ...
                  {appointment.date && appointment.time && (
                    <div className="flex items-center justify-between pt-2 border-t border-border/50">
                      ...
                      <CountdownBadge date={appointment.date} time={appointment.time} />
                    </div>
                  )}
                </div>
              ))}
      ...
      {/* Booked appointments from this session */}
      {bookings.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <GlassCard className="p-6">
            <h3 className="text-base font-semibold text-foreground mb-4">Recently Booked</h3>
            <div className="space-y-3">
              {bookings.map((b, i) => (
                <div key={i} className="p-4 bg-card/40 backdrop-blur-sm rounded-xl border border-border space-y-3">
                  <div className="flex items-center gap-4">
